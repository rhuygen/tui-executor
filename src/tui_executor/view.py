...
class ConsoleOutput(QTextEdit):
    ...
    def append(self, text):
        ...
        console = Console(record=True, width=240, theme=theme)
        ...
        with MyClient(self._kernel) as client:
            ...
            if self.cmd_log:
                self._console_panel.append(
                    f"Loading [blue]tui_executor.transforms[/] extension...log file in '{self.cmd_log}'")
                client.run_snippet(
                    textwrap.dedent(
                        f"""\
                        from tui_executor import transforms
                        transforms.set_log_file_location("{self.cmd_log}")
                        %load_ext tui_executor.transforms
                        """
                    )
                )
