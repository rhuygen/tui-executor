...
def var_exists(var_name: str) -> bool:
    """
    Returns True if the variable with the given name exists in the locals() or globals() namespace.
    """
...
def stringify_var_name_checks(args, kwargs):
    from tui_executor.utypes import var_name
    return "\n".join(
...
def create_code_snippet(func: Callable, args: List, kwargs: Dict, call_func: bool = True):
    ...
    # [3405691582] magic number is defined in tui_executor.transforms
...
def select_directory(directory: str = None) -> str | None:
    """
    Show a file selector dialog and allow the user to select a directory.

    Args:
        directory: the name of the starting folder

    Returns:
        The name of the selected folder or None.
    """
...
def select_file(filename: str = None, full_path: bool = True) -> str | None:
    """
    Show a file selector dialog and allow the user to select a file.

    Args:
        filename: the name of a file that will be selected as a starting point.
        full_path: True if the full path is required, False for the basename.

    Returns:
        The name of the selected file or None.
    """
